...
class TestAPIEndpoints(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_submit_url_cache_hit(self, mock_get_by_url, mock_redis_get):
        # Mock Redis returns cached image URL (cache hit)
        ...
        mock_get_by_url.assert_awaited_once()

    @patch("api.routes.REDIS_CLIENT.get", new_callable=AsyncMock)
    @patch("database.crud.get_url_entry_by_url", new_callable=AsyncMock)
    @patch("services.og_scraper.cache_save", new_callable=AsyncMock)
    async def test_submit_url_existing_entry_with_image(self, mock_cache_save, mock_get_by_url, mock_redis_get):
    ...
    async def test_submit_url_new_entry(
        self, mock_process, mock_get_by_id, mock_create, mock_get_by_url, mock_redis_get
    ):
        mock_redis_get.return_value = None
    ...
    async def test_get_history(self, mock_get_all):
